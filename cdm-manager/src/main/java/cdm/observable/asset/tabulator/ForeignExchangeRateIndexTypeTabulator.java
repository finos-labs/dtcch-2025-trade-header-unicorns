...
@ImplementedBy(ForeignExchangeRateIndexTypeTabulator.Impl.class)
public interface ForeignExchangeRateIndexTypeTabulator extends Tabulator<ForeignExchangeRateIndex> {
	@Singleton
	class Impl implements ForeignExchangeRateIndexTypeTabulator {
		...
		private final Field primaryFxSpotRateSourceField;
		private final Field secondaryFxSpotRateSourceField;
		...
		@Inject
		public Impl(AssetIdentifierTypeTabulator assetIdentifierTypeTabulator, TaxonomyTypeTabulator taxonomyTypeTabulator, LegalEntityTypeTabulator legalEntityTypeTabulator, QuotedCurrencyPairTypeTabulator quotedCurrencyPairTypeTabulator, InformationSourceTypeTabulator informationSourceTypeTabulator) {
			...
			this.primaryFxSpotRateSourceField = new FieldImpl(
			...
			this.secondaryFxSpotRateSourceField = new FieldImpl(
		...
		@Override
		public List<FieldValue> tabulate(ForeignExchangeRateIndex input) {
			...
			FieldValue primaryFxSpotRateSource = Optional.ofNullable(input.getPrimaryFxSpotRateSource())
				.map(x -> new NestedFieldValueImpl(primaryFxSpotRateSourceField, Optional.of(informationSourceTypeTabulator.tabulate(x))))
				.orElse(new NestedFieldValueImpl(primaryFxSpotRateSourceField, Optional.empty()));
			FieldValue secondaryFxSpotRateSource = Optional.ofNullable(input.getSecondaryFxSpotRateSource())
				.map(x -> new NestedFieldValueImpl(secondaryFxSpotRateSourceField, Optional.of(informationSourceTypeTabulator.tabulate(x))))
				.orElse(new NestedFieldValueImpl(secondaryFxSpotRateSourceField, Optional.empty()));
