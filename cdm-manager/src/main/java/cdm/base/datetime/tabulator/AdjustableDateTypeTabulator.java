...
@ImplementedBy(AdjustableDateTypeTabulator.Impl.class)
public interface AdjustableDateTypeTabulator extends Tabulator<AdjustableDate> {
	@Singleton
	class Impl implements AdjustableDateTypeTabulator {
		private final Field unadjustedDateField;
		private final Field dateAdjustmentsField;
		private final Field dateAdjustmentsReferenceField;
		...
		public Impl(BusinessDayAdjustmentsTypeTabulator businessDayAdjustmentsTypeTabulator) {
			...
			this.dateAdjustmentsReferenceField = new FieldImpl(
		...
		@Override
		public List<FieldValue> tabulate(AdjustableDate input) {
			...
			FieldValue dateAdjustmentsReference = Optional.ofNullable(input.getDateAdjustmentsReference())
				.map(x -> x.getValue())
				.map(x -> new NestedFieldValueImpl(dateAdjustmentsReferenceField, Optional.of(businessDayAdjustmentsTypeTabulator.tabulate(x))))
				.orElse(new NestedFieldValueImpl(dateAdjustmentsReferenceField, Optional.empty()));
