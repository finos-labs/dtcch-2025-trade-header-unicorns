...
@ImplementedBy(AdjustedRelativeDateOffsetTypeTabulator.Impl.class)
public interface AdjustedRelativeDateOffsetTypeTabulator extends Tabulator<AdjustedRelativeDateOffset> {
	@Singleton
	class Impl implements AdjustedRelativeDateOffsetTypeTabulator {
		...
		private final Field relativeDateAdjustmentsField;
		...
		@Inject
		public Impl(BusinessCentersTypeTabulator businessCentersTypeTabulator, BusinessDayAdjustmentsTypeTabulator businessDayAdjustmentsTypeTabulator) {
			...
			this.relativeDateAdjustmentsField = new FieldImpl(
		...
		@Override
		public List<FieldValue> tabulate(AdjustedRelativeDateOffset input) {
			...
			FieldValue relativeDateAdjustments = Optional.ofNullable(input.getRelativeDateAdjustments())
				.map(x -> new NestedFieldValueImpl(relativeDateAdjustmentsField, Optional.of(businessDayAdjustmentsTypeTabulator.tabulate(x))))
				.orElse(new NestedFieldValueImpl(relativeDateAdjustmentsField, Optional.empty()));
